...
pub mod m0011_edition_format_metadata;
pub mod schema;
...
impl MigratorTrait for Migrator {
    ...
    fn migrations() -> Vec<Box<dyn MigrationTrait>> {
        vec![
            ...
            Box::new(crate::migration::m0011_edition_format_metadata::Migration),
        ]
    }
}
...
impl Migrator {
    pub async fn run(pool: &sqlx::SqlitePool) -> Result<(), sqlx::Error> {
        ...
        // Refresh query-planner statistics after the schema/seed settles
        // so the optimizer can pick the new indexes immediately.
