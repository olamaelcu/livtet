...
/// Run the plugin host process. Takes CLI arguments as a `Vec<String>`
/// (the caller is responsible for parsing via `std::env::args()`).
/// This is extracted from `main()` so downstream crates can embed the
/// same binary with their own `[[bin]]` entry — Cargo compiles it as
/// a first-class binary in the same target directory.
pub async fn run(args: Vec<String>) -> ExitCode {
    ...
    let router: crate::ipc_host::CallbackRouter =
