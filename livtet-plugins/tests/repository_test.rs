...
fn test_installed_file_round_trip() {
    ...
    file.upsert(livtet_plugins::repository::installed::entry_for(
        "openlibrary",
        "1.2.3",
...
fn test_installed_file_tamper_detected() {
    ...
    let result = livtet_plugins::repository::installed::InstalledFile::load(&installed_path, &key);
    assert!(
        result.is_err(),
...
use livtet_plugins::repository::publisher::{
...
fn test_sign_index_re_signs_existing() {
    ...
    fs::write(
        ...
        livtet_plugins::repository::index::render_index_json(&index),
    )
    .unwrap();
...
fn test_sign_index_errors_when_index_json_missing() {
    ...
    match err {
        livtet_plugins::repository::error::RepositoryError::NotFound(name) => {
...
fn test_parse_index_json_rejects_unsupported_format_version() {
    ...
    let err = livtet_plugins::repository::index::parse_index_json(bad)
