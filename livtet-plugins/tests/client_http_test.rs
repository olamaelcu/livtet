...
async fn test_fetch_repo_toml_404() {
    ...
    match result {
        Err(livtet_plugins::repository::error::RepositoryError::Http { status, .. }) => {
...
async fn test_fetch_index_rejects_bad_signature() {
    ...
    match result {
        Err(livtet_plugins::repository::error::RepositoryError::BadIndexSignature) => {}
...
async fn test_download_archive_rejects_size_mismatch() {
    ...
    let result = client
        ...
        .await;
    match result {
        Err(livtet_plugins::repository::error::RepositoryError::Network(msg)) => {
...
async fn test_download_archive_rejects_sha256_mismatch() {
    ...
    let result = client
        ...
        .await;
    match result {
        Err(livtet_plugins::repository::error::RepositoryError::IndexParse(msg)) => {
...
async fn test_e2e_live_http_round_trip() {
    ...
    let verifying_key = signing_key.verifying_key();
    let publisher_fp = livtet_plugins::keys::fingerprint(&verifying_key);
...
async fn test_confirm_add_falls_back_to_fingerprint_lookup() {
    ...
    let verifying_key = signing_key.verifying_key();
    let repo_fp = livtet_plugins::keys::fingerprint(&verifying_key);
...
async fn test_confirm_add_rejects_unknown_fingerprint() {
    ...
    match result {
        Err(livtet_plugins::repository::error::RepositoryError::Keyring(msg)) => {
            assert!(
...
async fn test_update_same_fingerprint_refreshes_index() {
    ...
    let parsed = livtet_plugins::repository::index::parse_index_json(&cached_index).unwrap();
    assert!(
...
async fn test_confirm_update_returns_http_error_on_4xx() {
    ...
    match err {
        livtet_plugins::repository::error::RepositoryError::Http { status, .. } => {
