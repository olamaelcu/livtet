...
fn memory_limit_is_set() {
    ...
    let loaded = match result {
        livtet_plugin::protocol::HostToMain::PluginLoaded { load_state, .. } => load_state,
...
fn memory_limit_rejects_runaway_allocation() {
    ...
    let result = host
        ...
        .expect("LoadPlugin returns Some(_)");
    match result {
        livtet_plugin::protocol::HostToMain::PluginLoadError { error, .. } => {
            assert!(
...
fn resolved_fs_grant(
    read_globs: &[&str],
    write_globs: &[&str],
) -> std::sync::Arc<livtet_plugin::permissions::ResolvedGrant> {
    ...
    std::sync::Arc::new(livtet_plugin::permissions::ResolvedGrant {
        raw: livtet_plugin::permissions::PluginGrant {
            version: 1,
