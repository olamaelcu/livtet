...
use livtet_lua_plugins::{BundledPlugin, embedded_index, read_entry};
use livtet_plugin::{
    host_lua::LuaHost,
    host_trait::{
...
/// `EmbeddedHost` (see `crates/livtet-plugin/src/embedded_host.rs`).
struct MockHost {
...
struct MockHostInner {
    ...
    system_secrets:
        std::collections::HashMap<livtet_plugin::system_secrets::PluginSystemSecret, String>,
}
...
impl HostDatabase for MockHost {
    ...
    fn fetch_progress(
        &self,
        _urn: &str,
    ) -> Result<Option<livtet_plugin::progress_entry::ProgressEntry>, HostError> {
        Err(HostError::Unsupported)
    }
    fn upsert_progress(
        &self,
...
/// Load the bundled plugin source identified by `id` via the
/// `livtet-lua-plugins` crate's runtime index, parse the
/// manifest, and resolve the entry file. Mirrors the
/// `init_plugins` flow in `crates/ffi/livtet-ffi/src/lookup.rs`.
fn load_bundled_source(plugin_id: &str) -> String {
    let idx = embedded_index();
    let plugin: &BundledPlugin = idx
        .get(plugin_id)
        .unwrap_or_else(|| panic!("bundled plugin {plugin_id:?} not in embedded index"));
    let manifest_str = std::str::from_utf8(plugin.manifest_bytes)
        .unwrap_or_else(|e| panic!("manifest for {plugin_id} is not UTF-8: {e}"));
    let manifest: PluginManifest = toml::from_str(manifest_str)
        .unwrap_or_else(|e| panic!("manifest for {plugin_id} parse failed: {e}"));
    let entry = &manifest.plugin.entry;
    read_entry(plugin, entry)
        .unwrap_or_else(|| panic!("entry {entry} not found in bundled plugin {plugin_id}"))
}
