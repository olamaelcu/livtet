...
use fs_err as fs;
use livtet_plugin::repository::hmac::HmacKey;
...
pub fn verifying_key_from_keygen_report(
    report: &livtet_plugin::types::KeygenReport,
) -> ed25519_dalek::VerifyingKey {
    let text = fs_err::read_to_string(&report.pubkey_path).expect("read pubkey");
    livtet_plugin::keys::signing::parse_pubkey_text(&text).expect("parse_pubkey_text")
}

use std::time::Duration;

use camino::{Utf8Path, Utf8PathBuf};
use livtet_plugin::host_manager::PluginHostManager;
