...
use common::as_utf8;
use livtet_plugin::keys::passphrase::{PassphraseSource, resolve_passphrase};
...
use livtet_plugin::keys::TrustStore;
...
fn test_keygen_writes_minisign_box_format() {
    ...
    let parsed_pk = livtet_plugin::keys::signing::parse_pubkey_text(&pk_text)
...
use livtet_plugin::keys::signing::parse_pubkey_text;
...
fn test_parse_pubkey_text_auto_parses_livtet_custom_format() {
    ...
    let actual = livtet_plugin::keys::fingerprint(&parsed);
...
fn test_load_encrypted_key_with_correct_passphrase() {
    ...
    .unwrap();
    assert!(report.encrypted);
    let result = livtet_plugin::keys::signing::load_minisign_signing_key(&report.key_path);
    assert!(
        result.is_ok(),
...
use livtet_plugin::keys::parse_pubkey_b64;
...
use livtet_plugin::types::TrustedKeySource;
...
mod bundled_trust {
    use ed25519_dalek::SigningKey;
    use livtet_lua_plugins::BUNDLED_SIGNER_PUB_TEXT;
    use livtet_plugin::keys::{
        bundled_trusted_keys, fingerprint,
        signing::{parse_pubkey_text, sign_bytes, verify_bytes},
    };
    use rand::{Rng as _, rng};
    ...
    #[test]
    fn bundled_pubkey_parses_as_ed25519_verifying_key() {
        let vk = parse_pubkey_text(BUNDLED_SIGNER_PUB_TEXT)
            .expect("bundled signer.pub must parse as a minisign public key box");
        // Round-trip via TrustStore::load to confirm it flows through.
        // Without the `bundled` cargo feature the trust store will be
        // empty; the test only enforces parsing + round-trip then.
        let store = TrustStore::load().expect("TrustStore::load must succeed");
        let fp = fingerprint(&vk);

        if bundled_trusted_keys().is_empty() {
            // Built without `bundled` feature — just confirm parsing works.
            assert!(!store.is_trusted(&vk));
            assert_eq!(fp.len(), "SHA256:".len() + 64);
        } else {
            assert!(
                store.is_trusted(&vk),
                "bundled signer must be recognized as trusted after TrustStore::load"
            );
            let builtin_entries: Vec<_> = store
                .list_trusted()
                .into_iter()
                .filter(|k| k.source == TrustedKeySource::Builtin)
                .filter(|k| k.fingerprint == fp)
                .collect();
            assert_eq!(
                builtin_entries.len(),
                1,
                "bundled signer should appear exactly once in the builtin trust list"
            );
            assert_eq!(builtin_entries[0].label, "livtet-lua-plugins-signer");
        }
    }

    #[test]
    fn bundled_key_rejects_unrelated_signatures() {
        // Smoke test that the parsed pubkey isn't garbage: it must
        // reject signatures produced by a different key. (We can't
        // sign with the bundled private key itself because the private
        // key lives only in .mise/secrets.json.sops — not in this
        // crate.)
        let bundled_vk = parse_pubkey_text(BUNDLED_SIGNER_PUB_TEXT).unwrap();
        let bogus_key = make_key();
        let bogus_sig = sign_bytes(&bogus_key, b"any message").unwrap();
        assert!(verify_bytes(&bundled_vk, b"any message", &bogus_sig).is_err());
    }
}
