...
use serde_json::json;
use livtet_database::sql::{AssertSqlSafe, Connection};
...
impl HostProcess {
    fn spawn() -> Self {
        let child = Command::new(env!("CARGO_BIN_EXE_livtet-plugin-host-lua"))
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::piped())
            .spawn()
...
fn load_probe_with_perms_dir(perms_dir: &Utf8Path) -> (HostProcess, TempDir) {
    let mut command = Command::new(env!("CARGO_BIN_EXE_livtet-plugin-host-lua"));
    command
...
fn build_sample_sqlite(dir: &Utf8Path) -> camino::Utf8PathBuf {
    ...
    run_async(async {
        let options = livtet_database::sql::sqlite::SqliteConnectOptions::new()
            .filename(path.as_std_path())
            .create_if_missing(true);
        let mut conn = livtet_database::sql::sqlite::SqliteConnection::connect_with(&options)
            .await
            .expect("open sqlite");
        livtet_database::sql::query(AssertSqlSafe("CREATE TABLE t (a INTEGER, b TEXT);"))
            .execute(&mut conn)
            .await
            .expect("create table");
        livtet_database::sql::query(AssertSqlSafe(
...
fn host_sqlite_query_caps_result_rows() {
    ...
    run_async(async {
        ...
        livtet_database::sql::query(AssertSqlSafe("CREATE TABLE big (x INTEGER);"))
            .execute(&mut conn)
            .await
        ...
        for i in 0..12_000i64 {
            livtet_database::sql::query(AssertSqlSafe("INSERT INTO big (x) VALUES (?1)"))
                .bind(i)
                .execute(&mut conn)
                .await
                .expect("insert row");
