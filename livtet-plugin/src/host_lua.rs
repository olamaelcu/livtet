...
use livtet_database::sql::AssertSqlSafe;
        ...
        let require =
            self.lua
                .create_function(move |lua, (target,): (String,)| -> mlua::Result<Value> {
                    ...
                    if let Some(bytes) = livtet_lua_stdlib::embedded_index().resolve(&target) {
                        let code = std::str::from_utf8(bytes).map_err(|e| {
                            mlua::Error::external(format!(
                                "bundled-lua rock {target:?} not UTF-8: {e}"
                            ))
                        })?;
                        let chunk = lua.load(code).set_name(format!("bundled-lua:{target}"));
                        let returns: mlua::MultiValue = chunk.call(())?;
                        let result = returns.into_iter().next().unwrap_or(Value::Nil);
                        if let Ok(mut g) = require_cache.lock() {
                            g.insert(target.clone(), result.clone());
                        }
                        return Ok(result);
                    }
                    Err(mlua::Error::external(format!(
                        "host.require: bundled-lua has no rock named {target:?}; \
                         declare `rocks = [\"{target}\"]` in livtet.toml so the \
                         parent can install it via luarocks and expose it \
                         through LUA_PATH"
                    )))
                })?;
...
fn run_sqlite_query(
    ...
    let rows_result: Result<Vec<SqliteRow>, _> =
        rt.block_on(async { livtet_database::sql::query(AssertSqlSafe(sql)).fetch_all(&pool).await });
