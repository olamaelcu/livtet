...
use livtet_database::sql::{self, AssertSqlSafe, SqlitePool};
use tokio::{
    io::AsyncWriteExt,
        ...
        loop {
            ...
            match msg {
                ...
                HostToMain::HttpRequest { .. } => {
                    $self.handle_http_request(msg).await;
                }
                msg @ $dispatch => {
...
#[cfg(feature = "bundled")]
use crate::discovery::scan_embedded_plugins;
use crate::{
...
///
/// The dispatcher in `livtet-plugin` doesn't depend on
...
pub struct PluginHostManager {
    ...
    /// When `true` (the default), every bundled (`PluginSource::Embedded`)
    /// plugin is loaded as part of `discover()`. Disk-installed plugins
    /// always require an explicit `load_plugin` call. Tests that need to
    /// control loading precisely can flip this off via the builder below.
    auto_load_bundled: bool,
    /// Trust store consulted during `register_discovered`. Bundled
    /// plugins (`PluginSource::Embedded`) are skipped with a warning
    /// if the embedded bundled key isn't trusted here. Disk-installed
...
/// catches the case where a Tauri caller (or test) instantiates
/// `PluginHostManager` with `TrustStore::empty()` (no builtin or
/// user key) and would otherwise load bundled plugins into the Lua
/// VM with no chain of trust at all.
#[cfg(feature = "bundled")]
fn bundled_signer_is_trusted(trust_store: &TrustStore) -> bool {
    use crate::keys::bundled_trusted_keys;
    bundled_trusted_keys()
        .into_iter()
        .any(|(_, vk)| trust_store.is_trusted(&vk))
}
...
impl PluginHostManager {
    ...
    async fn discover(&mut self, plugin_dir: &Utf8PathBuf) -> PluginResult<()> {
        ...
        // user-installed disk plugins with the same id shadow them
        // when both are discovered below.
        #[cfg(feature = "bundled")]
        let bundled_ids: Vec<String> = {
            let mut ids = Vec::new();
            for plugin in scan_embedded_plugins() {
                ids.push(plugin.id.clone());
        ...
        // Auto-load bundled plugins so that `enabled` and `load_state`
        // line up at startup. Disk-installed plugins stay explicit —
        // they need per-plugin data dirs and permission grants that
        // the user has to ack via the plugin manager UI.
        if self.auto_load_bundled {
            #[cfg(feature = "bundled")]
            for id in &bundled_ids {
                let Some(entry) = self.loaded_plugins.get(id) else {
                    continue;
                };
                let Some(version) = entry.versions.keys().next().cloned() else {
                    continue;
                };
                if let Err(e) = self.load_plugin(id, &version).await {
                    warn!(
                        plugin = %id,
                        version = %version,
                        error = %e,
                        "failed to auto-load bundled plugin; will remain discovered but unloaded"
                    );
                }
            }
        }
    ...
    fn register_discovered(&mut self, plugin: DiscoveredPlugin) {
        ...
        // Bundled plugins must be signed by a key the trust store
        // recognises. Without the `bundled` feature, this gate is
        // skipped (no embedded plugin source bytes to gate). With
        // the feature, an unrecognised signer means the bundled
        // signer.pub in the binary is missing or has been replaced —
        // either way we refuse to register rather than silently load
        // tampered code into the sidecar Lua VM.
        #[cfg(feature = "bundled")]
        if let PluginSource::Embedded = &plugin.source {
            if !bundled_signer_is_trusted(&self.trust_store) {
                warn!(
                    plugin = %id,
                    version = %version,
                    "bundled plugin skipped: embedded signer key not present in trust store \
                     (run `mise run plugin-key-rotate --force` and rebuild)"
                );
                return;
            }
        }
        ...
        let source = match plugin.source {
            ...
            PluginSource::Embedded => {
                let idx = livtet_lua_plugins::embedded_index();
                match idx.get(&id) {
                    Some(p) => match livtet_lua_plugins::read_entry(p, &entry_path) {
                        Some(src) => {
                            tracing::info!(
                                plugin = %id,
                                version = %version,
                                "loaded bundled plugin from memory"
                            );
                            src
                        }
                        None => {
                            warn!(
                                ...
                                version = %version,
                                entry = %entry_path,
                                "bundled plugin entry source is not valid UTF-8 or missing"
                            );
                            String::new()
                        }
                    },
                    None => {
                        warn!(
                            plugin = %id,
                            version = %version,
                            "bundled plugin not found in embedded index"
                        );
                        String::new()
                    }
                }
    ...
    /// Stub: the previous DB-backed implementation called into
    /// `livtet_core::crud::*` (deleted). For now the host silently
    /// drops `HttpRequest` messages; a follow-up can wire this to a
    /// generic IPC-bridge channel.
    #[allow(dead_code)]
    async fn handle_http_request(&self, _msg: HostToMain) {
        warn!("handle_http_request: stub; HttpRequest dropped");
    }

    /// Stub: the previous DB-backed implementation looked up the
    /// edition row directly. With `livtet_core::crud` gone, the
    /// host returns `EditionInfoResult { error: Some(...) }` so the
    /// caller fails fast instead of hanging on the oneshot.
    #[allow(dead_code)]
    async fn handle_get_edition_info(
        ...
        MainToHostCallback::EditionInfoResult {
            ...
            info: None,
            error: Some(
                "get_edition_info: removed alongside livtet_core::crud; \
                 wire the host bridge to a replacement before enabling"
                    .to_string(),
            ),
        }
    }
