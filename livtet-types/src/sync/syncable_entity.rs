...
/// ```
pub trait ChangeLogEntity: sea_orm::EntityTrait {
...
impl SyncableEntityKind {
    ...
    pub async fn dump_all(db: &sea_orm::DatabaseConnection) -> Result<super::EntityDump> {
        Ok(super::EntityDump {
...
pub fn entity_type_to_table(entity_type: &str) -> Option<&'static str> {
    // Use a simple match for efficiency (no iteration needed).
