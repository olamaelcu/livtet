...
mod tests {
    ...
    fn dberr_converts_via_from_into_db_variant() {
        // The `From<DbErr> for SyncError` impl delegates to
        // `ConstraintViolation::enhance_db_err`.  When the message cannot
        // be matched to a known constraint, the variant is `Db { message,
        // violation: None }` carrying the original error text.
        fn returns_dberr() -> std::result::Result<(), sea_orm::DbErr> {
            Err(sea_orm::DbErr::RecordNotFound("missing".to_string()))
        }
        ...
        // And via the explicit From impl, with no `?` involved.
        let dberr = sea_orm::DbErr::Custom("explicit".to_string());
