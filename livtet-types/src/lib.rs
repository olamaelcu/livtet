...
//! - `uniffi`: enables UniFFI `Enum`/`Record` derives and fail-closed
//!   string converters (`DbId`, `DiskPath`, `Isbn`, `Urn`, `Identifier`)
//!   so these types cross the FFI boundary directly. Used by `livtet-ffi`.
...
#[cfg(feature = "uniffi")]
/// Marker type UniFFI's derive macros reference as `crate::UniFfiTag`
/// (def-site hygiene). Normally generated by `uniffi::setup_scaffolding!`;
/// defined here so this crate's derives compile without scaffolding.
#[doc(hidden)]
